 ...
 *
 * City of Coquitlam GIS (ArcGIS org Q6Lq3evZUGfPrN7o): parcels, zoning, parks & city services. search_datasets finds layers (each
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Arcgis Coquitlam');
}


const HUB = 'https://hub.arcgis.com/api/v3/datasets';
const UA = 'pipeworx-mcp-arcgis-coquitlam/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search City of Coquitlam GIS open geospatial datasets (parcels, zoning, parks & city services) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (City of Coquitlam GIS).' },
